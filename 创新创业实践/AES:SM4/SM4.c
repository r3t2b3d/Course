...
void build_data_32(char *data, uint32_t *_data){
    for(int i=0; i<4; i++){
        _data[i] = (data[4*i]<<24)+(data[4*i+1]<<16)\
                    +(data[4*i+2]<<8)+(data[4*i+3]);
    }
}

void rebuild_data_32(char *data, uint32_t *_data){
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            data[4*i+j] = _data[i]>>(24-8*j);
        }
    }
}

char m[] = "it's a test text";
char k[] = "it's a test key!";
char buf[16];
