...
if __name__ == "__main__":
    ...
    def rho_burp(seed):
        ...
        while h0 != h1:
            ...
            h1 = f(f(h1))
            chain.append(h0)
        while f(h0) not in chain:
            h0 = f(h0)
            chain.append(h0)
        ch1 = chain[chain.index(f(h0))-1]
        assert f(ch1) == f(h0)
        assert ch1 != h0
        print("[+]Find collision!")
        print("{} and {} have same hash value {}".format(ch1, h0, f(h0)))
    start = time.time()
    rho_burp('0')
    end = time.time()
...
"""
[+]Find collision!
dd247a3b1b and 6a6884cb1d have same hash value 4f5b6e8b45
cost 297.9176721572876 s
"""